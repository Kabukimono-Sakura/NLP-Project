...
# Run only the prompt-based PMP method on SemEval-200 and/or SemEval-784.
# This script uses the original base model and the Full PMP prompt.
...
run_prediction() {
  ...
  echo "[RUN] $name PMP prediction -> $output"
  echo "      model : $BASE_MODEL"
  echo "      prompt: $FRAMEWORK1_PROMPT"

  "$PYTHON_BIN" framework3/scripts/run_llm_predictions.py \
    --input "$input" \
    --output "$output" \
    --model "$BASE_MODEL" \
    --prompt-file "$FRAMEWORK1_PROMPT" \
    --id-column "$id_col" \
...
run_dataset() {
  ...
  local pred="$PRED_DIR/${tag}_framework1.jsonl"
  local eval="$RESULT_DIR/${tag}_framework1_eval.json"

  echo "=========================================="
  echo "PMP dataset    : $tag"
  echo "Input          : $input"
  echo "Model          : $BASE_MODEL"
  echo "Max new tokens : $MAX_NEW_TOKENS"
  echo "=========================================="

  run_prediction "${tag}/framework1" "$input" "$pred" "$id_col" "$text_col" "$gold_col"
  run_eval "${tag}_framework1" "$pred" "$eval"
}
...
echo "=========================================="
echo "PMP framework1 run finished"
echo "Predictions:"
find "$PRED_DIR" -maxdepth 1 -type f -name "*_framework1.jsonl" -print | sort
echo "Results:"
find "$RESULT_DIR" -maxdepth 1 -type f -name "*_framework1_eval.json" -print | sort
echo "=========================================="
